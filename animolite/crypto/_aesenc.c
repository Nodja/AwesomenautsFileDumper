...
/*  0 */ _CFFI_OP(_CFFI_OP_FUNCTION, 12), // void()(uint8_t *, uint8_t *)
/*  1 */ _CFFI_OP(_CFFI_OP_POINTER, 9), // uint8_t *
/*  2 */ _CFFI_OP(_CFFI_OP_NOOP, 1),
/*  3 */ _CFFI_OP(_CFFI_OP_FUNCTION_END, 0),
/*  4 */ _CFFI_OP(_CFFI_OP_FUNCTION, 12), // void()(uint8_t *, uint8_t *, uint8_t(*)[16])
/*  5 */ _CFFI_OP(_CFFI_OP_NOOP, 1),
/*  6 */ _CFFI_OP(_CFFI_OP_NOOP, 1),
/*  7 */ _CFFI_OP(_CFFI_OP_POINTER, 10), // uint8_t(*)[16]
/*  8 */ _CFFI_OP(_CFFI_OP_FUNCTION_END, 0),
/*  9 */ _CFFI_OP(_CFFI_OP_PRIMITIVE, 18), // uint8_t
/* 10 */ _CFFI_OP(_CFFI_OP_ARRAY, 9), // uint8_t[16]
/* 11 */ (_cffi_opcode_t)(16),
/* 12 */ _CFFI_OP(_CFFI_OP_PRIMITIVE, 0), // void
};

static void _cffi_d_encrypt(uint8_t * x0, uint8_t * x1, uint8_t(* x2)[16])
{
  encrypt(x0, x1, x2);
}
#ifndef PYPY_VERSION
static PyObject *
_cffi_f_encrypt(PyObject *self, PyObject *args)
{
  uint8_t * x0;
  uint8_t * x1;
  uint8_t(* x2)[16];
  ...
  aa = _cffi_unpack_args(args, 3, "encrypt");
  if (aa == NULL)
    return NULL;
  arg0 = aa[0];
  arg1 = aa[1];
  ...
  if (datasize != 0) {
    if (datasize < 0)
      return NULL;
    x2 = (uint8_t(*)[16])alloca((size_t)datasize);
...
#else
#  define _cffi_f_encrypt _cffi_d_encrypt
#endif

static void _cffi_d_fast_xor(uint8_t * x0, uint8_t * x1)
{
  fast_xor(x0, x1);
}
#ifndef PYPY_VERSION
static PyObject *
_cffi_f_fast_xor(PyObject *self, PyObject *args)
{
  uint8_t * x0;
  uint8_t * x1;
  Py_ssize_t datasize;
  ...
  aa = _cffi_unpack_args(args, 2, "fast_xor");
  if (aa == NULL)
    return NULL;
  arg0 = aa[0];
  arg1 = aa[1];
...
#else
#  define _cffi_f_fast_xor _cffi_d_fast_xor
#endif

static const struct _cffi_global_s _cffi_globals[] = {
  { "encrypt", (void *)_cffi_f_encrypt, _CFFI_OP(_CFFI_OP_CPYTHON_BLTN_V, 4), (void *)_cffi_d_encrypt },
  { "fast_xor", (void *)_cffi_f_fast_xor, _CFFI_OP(_CFFI_OP_CPYTHON_BLTN_V, 0), (void *)_cffi_d_fast_xor },
};
...
#ifdef PYPY_VERSION
PyMODINIT_FUNC
_cffi_pypyinit__aesenc(const void *p[])
{
...
#  ifdef _MSC_VER
     PyMODINIT_FUNC
#  if PY_MAJOR_VERSION >= 3
     PyInit__aesenc(void) { return NULL; }
#  else
     init_aesenc(void) { }
#  endif
#  endif
...
PyInit__aesenc(void)
{
  return _cffi_init("_aesenc", 0x2601, &_cffi_type_context);
}
#else
PyMODINIT_FUNC
init_aesenc(void)
{
  _cffi_init("_aesenc", 0x2601, &_cffi_type_context);
}
#endif
